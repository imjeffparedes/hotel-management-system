...
Public Class frmAttendance
    ...
    Private Sub EmployeeID_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        Try
            Delete.Enabled = True
    ...
    Private Sub Delete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Delete.Click
        Try
            ...
            If EmployeeID.Text = "" Then
                MessageBox.Show("Please retrieve employee id", "Entry", MessageBoxButtons.OK, MessageBoxIcon.Warning)
                Exit Sub
            End If
    ...
    Sub GetData()
        Try
            con = New OleDbConnection(cs)
            con.Open()
            cmd = New OleDbCommand("SELECT (EmployeeRegistration.EmployeeName) as [Employee Name],(EmployeeRegistration.EmployeeID) as [Employee ID],EmployeeRegistration.BasicWorkingTime as [Basic Working Time] FROM EmployeeRegistration left join EmployeeAttendance on EmployeeRegistration.EmployeeID=EmployeeAttendance.EmployeeID group by EmployeeRegistration.employeename,employeeregistration.employeeid,EmployeeRegistration.BasicWorkingTime order by EmployeeName", con)
    ...
    Private Sub DataGridView1_RowHeaderMouseClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellMouseEventArgs) Handles DataGridView1.RowHeaderMouseClick
        Try
            ...
            Me.BasicWorkingTime.Text = dr.Cells(2).Value.ToString()
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
    ...
    Private Sub txtEmployee_TextChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles txtEmployee.TextChanged
        Try
            con = New OleDbConnection(cs)
            con.Open()
            cmd = New OleDbCommand("SELECT (EmployeeRegistration.EmployeeName) as [Employee Name],(EmployeeRegistration.EmployeeID) as [Employee ID],EmployeeRegistration.BasicWorkingTime as [Basic Working Time] FROM EmployeeRegistration left join EmployeeAttendance on EmployeeRegistration.EmployeeID=EmployeeAttendance.EmployeeID where EmployeeName like '" & txtEmployee.Text & "%' group by EmployeeRegistration.employeename,employeeregistration.employeeid,EmployeeRegistration.BasicWorkingTime order by EmployeeName", con)
            Dim myDA As OleDbDataAdapter = New OleDbDataAdapter(cmd)
            Dim myDataSet As DataSet = New DataSet()
            myDA.Fill(myDataSet, "EmployeeAttendance")
            DataGridView1.DataSource = myDataSet.Tables("EmployeeAttendance").DefaultView()
